...
class CustomDropDownMenuScreen extends StatefulWidget {
  @override
  _CustomDropDownMenuScreenState createState() => _CustomDropDownMenuScreenState();
}

class _CustomDropDownMenuScreenState extends State<CustomDropDownMenuScreen> {
      ...
      child: Center(
        child: DropdownMenu<ExampleItem>(
          ...
          itemBuilder: (BuildContext context) {
            return List<PopupMenuEntry<ExampleItem>>.generate(ExampleItem.values.length * 2 - 1,
                (int index) {
                if (index.isEven) {
...
class _DropdownMenuState<T> extends State<DropdownMenu<T>> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _rotation = Tween<double>(begin: 0.0, end: 180.0).animate(
      CurvedAnimation(
        parent: _controller, curve: Curves.elasticOut, reverseCurve: Curves.easeInExpo),
    );
  }
